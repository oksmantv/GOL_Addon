...
[QGVAR(AutoLock), "CHECKBOX", ["Auto Lock Vehicles", "Note: Only effects vehicles with units in and spawned though the framework"], [QUOTE(ADDON), "Spawning"], true, CBA_SERVEROVERWRITE] call CBA_settings_fnc_init;
[QGVAR(autoDelete), "CHECKBOX", ["Auto Delete Forgotten Units", "Automaticly deletes units and objects located at [0,0,0] after 10 seconds if not moved"], [QUOTE(ADDON), "Spawning"], true, CBA_SERVEROVERWRITE] call CBA_settings_fnc_init;
[QGVAR(autoQueue), "CHECKBOX", ["Queue Spawnlist", "Queue spawned units to prevent lag and will autotmaticly delay next group until current is finished"], [QUOTE(ADDON), "Spawning"], true, CBA_SERVEROVERWRITE] call CBA_settings_fnc_init;
