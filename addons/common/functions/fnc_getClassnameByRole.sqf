/*
	...
	Usage:
	["r", west] call GW_Common_Fnc_getClassnameByRole;
	["sl", east] call GW_Common_Fnc_getClassnameByRole;
...
switch (toLower _role) do {
	...
	case "mmg": {
		// Machine Gunner
		private _suffix = "HeavyGunner_F";
		if(_side isEqualTo independent) then {
			_suffix = "support_MG_F";
		};
		_classname = _sidePrefix + _suffix;
	};
	case "crew": {
		// Crewman
	...
	case "lr": {
		// Long Range / Spotter
		_classname = _sidePrefix + "spotter_F";
	};
	case "aa": {
		// Anti-Air Specialist
