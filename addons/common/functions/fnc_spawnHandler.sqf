...
if !((count _unitArray) isEqualTo 0) then {
	{
		...
		if (_unitPos isEqualType "") then {
			diag_log format ["[GW][Common][Static] unit=%1 unitPosType=STRING unitPos=%2 waypoints=%3 -> firing disableAICommand", _unit, _unitPos, (count _waypointArray)];
		...
		if !(_unitPos isEqualType "") then {
			diag_log format ["[GW][Common][Static] unit=%1 unitPosType=%2 -> disableAICommand skipped", _unit, (typeName _unitPos)];
		};
...
if !(_waypointArray isEqualTo []) then {
	...
	diag_log format ["[GW][Common][Static] group=%1 has no waypoints -> marked static", _group];
};
