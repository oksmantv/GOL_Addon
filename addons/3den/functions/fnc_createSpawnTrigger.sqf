...
/*
	Creates a server-only SpawnList trigger at the current 3DEN mouse/map position.
	- Trigger name: SpawnTrigger_X (X enumerated)
	- Trigger text: Spawn Trigger X
	- Activation: ANYPLAYER PRESENT
	- Condition: mounted check (legacy string)
...
private _condition = "call{!({(isNull (objectParent _x))} count thislist isEqualTo 0) && isServer}";
private _onActivation = format ["call{null = [%1] spawn GW_Fnc_spawnList;}", _xNum];

collect3DENHistory {
	_trg set3DENAttribute ["text", format ["Spawn Trigger %1", _xNum]];
...
if (!isNil "OKS_fnc_EdenRememberLastAction") then {
	["GW_3DEN_fnc_createSpawnTrigger", [], []] call OKS_fnc_EdenRememberLastAction;
};
