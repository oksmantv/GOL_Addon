...
private _forceFactionLower = toLower _forceFaction;

diag_log format ["[GW_Gear][Handler] start unit=%1 type=%2 local=%3 role=%4 forceFaction=%5", _unit, typeOf _unit, local _unit, _role, _forceFaction];
...
if (_isMan) then {
	...
	#include "\x\gw\addons\gear\scripts\Common.sqf"
	#include "\x\gw\addons\gear\scripts\factions_standard.sqf"
	#include "\x\gw\addons\gear\scripts\isNilCheck.hpp"

	if (GETSIDE(_unit) == 0) then {
		_UAVTerminal = "O_UavTerminal";
	};
	if (GETSIDE(_unit) == 2) then {
		_UAVTerminal = "I_UavTerminal";
	};

	if (_isCivilian) then {
		diag_log format ["[GW_Gear][Handler] civilian branch unit=%1 role=%2", _unit, _role];
		...
		diag_log format ["[GW_Gear][Handler] unit=%1 role=%2 loadoutFile=%3 player=%4 switchable=%5", _unit, _role, _loadoutFile, _isPlayer, (_unit in switchableUnits)];

		if !(missionNamespace getVariable ["GW_Gear_Debug_LoggedDefaultFns", false]) then {
			private _hasDefault = !(isNil "GW_Gear_fnc_DefaultCode");
			private _hasDefaultAI = !(isNil "GW_Gear_fnc_DefaultAICode");
			private _defaultType = if (_hasDefault) then { typeName GW_Gear_fnc_DefaultCode } else { "NIL" };
			private _defaultAIType = if (_hasDefaultAI) then { typeName GW_Gear_fnc_DefaultAICode } else { "NIL" };
			private _defaultStr = if (_hasDefault) then { str GW_Gear_fnc_DefaultCode } else { "<nil>" };
			private _defaultAIStr = if (_hasDefaultAI) then { str GW_Gear_fnc_DefaultAICode } else { "<nil>" };
			private _defaultPreview = if ((count _defaultStr) > 120) then { _defaultStr select [0, 120] } else { _defaultStr };
			private _defaultAIPreview = if ((count _defaultAIStr) > 120) then { _defaultAIStr select [0, 120] } else { _defaultAIStr };

			diag_log format ["[GW_Gear][Handler] function probe DefaultCode has=%1 type=%2 preview=%3", _hasDefault, _defaultType, _defaultPreview];
			diag_log format ["[GW_Gear][Handler] function probe DefaultAICode has=%1 type=%2 preview=%3", _hasDefaultAI, _defaultAIType, _defaultAIPreview];

			missionNamespace setVariable ["GW_Gear_Debug_LoggedDefaultFns", true];
		};

		switch (_loadoutFile) do {
			case "Default_AI": {
				diag_log format ["[GW_Gear][Handler] executing DefaultAICode unit=%1 role=%2", _unit, _role];
				call GW_Gear_fnc_DefaultAICode;
			};
			default {
				diag_log format ["[GW_Gear][Handler] executing DefaultCode unit=%1 role=%2", _unit, _role];
				call GW_Gear_fnc_DefaultCode;
			};
		};
		diag_log format ["[GW_Gear][Handler] finished unit=%1 role=%2", _unit, _role];
	};
