...
diag_log format ["[GW_Gear][Default.sqf] start unit=%1 role=%2 loadoutFile=%3", _unit, _role, _loadoutFile];
...
if(_IRFlaresEnabled isEqualTo true && _glFlareW == "GOL_1Rnd_UGL_FlareWhite") then {
...
switch (toLower(_role)) do {
	...
	case "officer": {
		...
		[[_mapTools,1],[_HelmetCam,1],[_notepad,1],[_cables,6],[_gps,1],[_pistol_mag,2],[_smokegrenadeY,3],[_rifleC_mag,3]] call _addToUniform;
	...
	case "pm": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_mapTools,1],[_HelmetCam,1],[_notepad,1],[_gps,1],[_cables,6],[_pistol_mag,2],[_smokegrenadeY,4]] call _addToUniform;
	...
	case "fac": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_mapTools,1],[_HelmetCam,1],[_notepad,1],[_cables,6],[_gps,1],[_pistol_mag,2],[_grenade,2],[_smokegrenadeY,5]] call _addToUniform;
		[[_rifleGL_mag_tr,6],[_smokegrenadeB,5]] call _addToVest;
		[[_flashBang,3],[_battery,1],[_FacPanels,4]] call _addToBackpack;
		_laserDesignator call _addBino;
		[_map, "", "", _compass, _watch, ""] call _addLinkedItems;
		if (((call EFUNC(Common,isNight)) && _allowedNightStuff) || _ForceNightStuff) then {
			_nvg call _addNVG;
			[[_mapFlashLight,1],[_IRStrobe,1]] call _addToUniform;
			[[_chemB,12]] call _addToVest;
			[[_handFlareG,4],[_glFlareR,10],[_grenadeIR, 5],[_chemG,8]] call _addToBackpack;
		}
		else {
			[[_smokegrenadeY,1]] call _addToUniform;
			[[_smokegrenadeP,2],[_smokegrenadeB,6]] call _addToVest;
			[[_glsmokeG,2],[_smokegrenadeB,2],[_glsmokeR,5]] call _addToBackPack;
		};
		if(_ForceNVG isEqualTo true) then {
			_nvg call _addNVG;
		};	
	...
	case "sl": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_mapTools,1],[_HelmetCam,1],[_notepad,1],[_cables,6],[_gps,1],[_grenade,2],[_flashBang,2],[_pistol_mag,2],[_glsmokeR,5],[_smokegrenadeY,4],[_smokegrenadeB,3]] call _addToUniform;
		[[_grenade,2],[_rifleGL_mag_tr,5],[_glHEDP,7]] call _addToVest;
		[[_rifleGL_mag,4]] call _addToBackpack;
		_rangefinder call _addBino;
		[_map, "", "", _compass, _watch, ""] call _addLinkedItems;
	...
	case "sm": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_notepad,1],[_cables,6],[_pistol_mag,2],[_smokegrenadeY,4]] call _addToUniform;
		[[_smokegrenadeB,3],[_grenade,2],[_rifleC_mag_tr,6]] call _addToVest;
	...
	case "ftl": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_notepad,1],[_HelmetCam,1],[_cables,6],[_gps,1],[_flashBang,3],[_smokegrenadeB,3]] call _addToUniform;
		...
		if(_EntrenchAllow isEqualTo true) then {
			[[_entrenchingTool,1]] call _addToBackpack;
		};
		if(_ForceNVG isEqualTo true) then {
			_nvg call _addNVG;
		};
	...
	case "r": {
		...
		if (_useMineDetector) then {
			[[_minedetector,1]] call _addToUniform;
		};
	...
	case "g": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_cables,6],[_grenade,2],[_incendiaryGrenade,3],[_flashBang,3],[_pistol_mag,2],[_smokegrenadeY,3]] call _addToUniform;
	...
	case "engineer": {
		...
		_IFAK call _addToUniform;
		[[_HelmetCam,1],[_minedetector,1],[_cables,6],[_grenade,4],[_smokegrenadeY,2],[_defusalKit,1]] call _addToUniform;
	...
	case "ab": {
		...
		_rangefinder call _addBino;
		if(_EntrenchAllow isEqualTo true) then {
			[[_entrenchingTool,1]] call _addToVest;
		};
		if(_ForceNVG isEqualTo true) then {
			_nvg call _addNVG;
		};		
	...
	case "crew": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_gps,1],[_notepad,1],[_pistol_mag,2],[_smokegrenadeY,4],[_flashBang,2]] call _addToUniform;
	...
	case "mat": {
		...
		if (_useMineDetector) then {
			[[_minedetector,1]] call _addToBackpack;
		};
		if(_ForceNVG isEqualTo true) then {
			_nvg call _addNVG;
		};			
	...
	case "marksman": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_flashBang,3],[_notepad,1],[_gps,1],[_pistol_mag,2],[_rangecard,1],[_ATrag,1],[_Kestrel,1]] call _addToUniform;
	...
	case "pj": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_gps,1],[_notepad,1],[_pistol_mag,2],[_grenade,2]] call _addToUniform;
	...
	case "drone": {
		[_goggles,_helmet,_uniform,_vest,_backpack] call _addEquipment;
		[_rifleC, _rifleC_mag, ""] call _addPrimary;
		[_Disruptor_Pistol, _Disruptor_Mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
	...
	case "dragon": {
		...
		[_pistol, _pistol_mag, ""] call _addHandGun;
		_IFAK call _addToUniform;
		[[_gps,1],[_notepad,1],[_mortarRangeCard,1],[_mapTools,1]] call _addToUniform;
