...
if !(player diarySubjectExists QGVARMAIN(adminLogging)) then {
	player createDiarySubject [QGVARMAIN(adminLogging),"Admin Logging"];
};

player createDiaryRecord [QGVARMAIN(adminLogging), [format ["REMOTE: %1 - %2", _machine, str(time)], _strText]];
